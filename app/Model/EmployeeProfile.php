...
class EmployeeProfile extends Model
{
    ...
    protected $fillable = [
        'employee_profile_id', 'employee_id', 'branch_id', 'user_id', 'finger_id', 'department_id', 'designation_id', 'branch_id', 'work_shift', 'work_shift_id', 'supervisor_id', 'email', 'first_name',
        ...
        'photo', 'address', 'emergency_contacts', 'phone', 'status', 'created_by', 'updated_by', 'religion', 'device_employee_id', 'pf_status',
        'overtime_status', 'no_of_child', 'uan', 'cost_centre', 'pan_gir_no', 'salary_revision', 'salary_esi_stop', 'salary_ctc', 'salary_gross', 'bank_name', 'bank_account', 'bank_ifsc', 'emp_code', 'profile_status', 'functional_head_id',
        'document_title', 'document_name', 'document_expiry', 'document_title2', 'document_name2', 'document_expiry2', 'document_title3', 
        'document_name3', 'document_expiry3', 'document_title4', 'document_name4', 'document_expiry4', 
        'document_title5', 'document_name5', 'document_expiry5','document_title6', 'document_name6', 'document_expiry6'
    ];
    ...
    public function supervisorDetail()
    {
        if ($supervisor = $this->supervisor) {
            ...
            $supervisorDetail .= $supervisor->first_name . ' (' . $supervisor->emp_code . '';
            if ($roleId == 1) {
    ...
    public function detailname()
    {
        ...
        return $this->first_name . ' (' . $fullDetail . ')';
    }

    public function detail_name()
    {
        return $this->first_name . ' (' . $this->finger_id . ' - ' . ($this->branch ? $this->branch->branch_name : '') . ')';
    }
    ...
    public function supervisorIds($employee_id = '') // two level downline supervisor id
    {
        ...
        for ($i = 1; $i <= $this->maxDepth; $i++) {
            $temp = DB::table('employee')->select('employee_id')->whereIn('supervisor_id', $employee_id)->get();
            foreach ($temp as $key => $row) {
