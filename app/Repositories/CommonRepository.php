...
{
    public function leaveBalanceTypeList()
    {
    ...
    {
        // $results = LeaveType::get();
        $results = LeaveType::where('leave_type_id', '<=', 4)->get();
        $options = ['' => '---- Please select ----'];
        foreach ($results as $key => $value) {
            $options[$value->leave_type_id] = $value->leave_type_name;
        }
    ...
    {
        $results = LeaveType::where('leave_type_id', 2)->get();
        $options = ['' => '---- Please select ----'];
        foreach ($results as $key => $value) {
            $options[$value->leave_type_id] = $value->leave_type_name;
        }
    ...
    public function employeeList()
    {
        ...
        if ($branchId != null && $roleId != 1) {
            $results = Employee::where('status', 1)->where('branch_id', $branchId)->get();
        } else {
            $results = Employee::where('status', 1)->get();
        }
        $options = ['' => '---- Please select ----'];
        foreach ($results as $key => $value) {
            $options[$value->employee_id] = $value->detailname();
        }
        return $options;
    }

    public function nonMandatoryEmployeeList()
    {
        $branchId = session('logged_session_data.branch_id');
        $roleId = session('logged_session_data.role_id');
