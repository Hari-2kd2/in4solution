...
class WeeklyHolidayController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($branchId !== null && $roleId !== 1) {
            $count = Employee::where('branch_id', $branchId)->where('status', UserStatus::$ACTIVE)->count();
            $employeeList = Employee::where('branch_id', $branchId)->where('status', UserStatus::$ACTIVE)
            ...
            $count = Employee::where('branch_id', $selectedbranchId)->where('status', UserStatus::$ACTIVE)->count();
            $employeeList = Employee::where('branch_id', $selectedbranchId)->where('status', UserStatus::$ACTIVE)
                ->groupBy('department_id', 'finger_id')
                ->paginate($count);
            if (request()->ajax()) {
    ...
    public function store(WeeklyHolidayRequest $request)
    {
        ...
        try {
            ...
            foreach ($employees as $key => $value) {
                ...
                if (!$if_exists) {
                    WeeklyHoliday::createTrait($input);
                } else {
    ...
    public function importWeeklyHoliday(FileUploadRequest $request)
    {
        try {
            ...
            $import = new WeeklyHolidayImport($request->all());
            $import->import($file);
            ...
            return back()->with('error', 'Some rows failed validation. Check the failures for details.');
        }
    }
