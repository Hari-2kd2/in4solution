...
class TrainingReportController extends Controller
{
    ...
    {
        $results = TrainingLog::with(['employee', 'trainingInfo' => function ($q) {
            $q->with('trainingType');
        }])->get();
        ...
        if ($request->employee_id) {
            $results = TrainingLog::where('employee_id', $request->employee_id)->with(['employee', 'trainingInfo' => function ($q) {
                $q->with('trainingType');
            }])->get();
        }

        return view('admin.training.report.employeeTrainingReport', ['employeeList' => $employeeList, 'results' => $results, 'employee_id' => $request->employee_id]);
    }

    public function employeeTrainingReportOld(Request $request)
    {
        $employeeList = Employee::where('status', 1)->get();
        $data = [];
