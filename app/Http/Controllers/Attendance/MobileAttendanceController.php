...
class MobileAttendanceController extends Controller
{
    public function mobileAttendance(Request $request)
    {
        ...
        if ($_POST) {
            ...
            $filterBranchId = $request->post('branch_id');

            if ($roleId == 1) {
                $attendanceData = DB::table('view_employee_in_out_data')->join('employee', 'employee.finger_id', 'view_employee_in_out_data.finger_print_id')
                ...
                if ($filterBranchId) {
                    $attendanceData =    $attendanceData->where('view_employee_in_out_data.branch_id', $filterBranchId);
                }
                ...
                $attendanceData = DB::table('view_employee_in_out_data')->join('employee', 'employee.finger_id', 'view_employee_in_out_data.finger_print_id')
                    ...
                    ->join('branch', 'branch.branch_id', 'employee.branch_id')
                    ->where('employee.status', 1)
                    ->where('view_employee_in_out_data.branch_id', ($branchId))
                    ->where('view_employee_in_out_data.date', '>=', dateConvertFormtoDB($request->from_date))
                    ->where('view_employee_in_out_data.date', '<=', dateConvertFormtoDB($request->to_date))
                    ->whereIn('view_employee_in_out_data.device_name', ['Mobile', 'mobile'])
                    ->groupBy('view_employee_in_out_data.finger_print_id', 'view_employee_in_out_data.date')
            ...
            return view('admin.attendance.mobile.index', ['attendanceData' => $attendanceData, 'from_date' => $request->from_date, 'to_date' => $request->to_date, 'branchList' => $branchList]);
        }
    ...
    public function mobileAttendanceReport(Request $request)
    {
        ...
        $employeeInfo = DB::table('employee_attendance')->join('employee', 'employee.employee_id', 'employee_attendance.employee_id')
            ->join('department', 'department.department_id', 'employee.department_id')
            ...
            ->where('employee_attendance.in_out_time', '>=', date('Y-m-d 00:00:00', strtotime($date)))
            ->whereDate('view_employee_in_out_data.date', $date)
            ->where('employee_attendance.in_out_time', '<=', date('Y-m-d 23:59:59', strtotime($date)))
