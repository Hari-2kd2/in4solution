...
class GenerateSalarySheet extends Controller
{
    ...
    public function calculateEmployeeSalary(Request $request)
    {
        ...
        $employeeDetails = Employee::select('salary_ctc')->with('department', 'designation')->where('employee_id', $request->employee_id)->first();
        // $ttt = DB::table('employee')->where('employee_id', $request->employee_id)->value('salary_ctc'); 
        // dd($employeeDetails);
        // dd($ttt);
        ...
        if ($employeeDetails) {
            ...
            //     ->get();
            $leaveRecord = DB::table('leave_application')->select('leave_type.leave_type_id', 'leave_type_name', 'number_of_day', 'leave_application.application_from_date', 'leave_application.application_to_date')
                ->join('leave_type', 'leave_type.leave_type_id', 'leave_application.leave_type_id')
                ->where('status', LeaveStatus::$APPROVE)
                ->where('leave_application.application_from_date', '>=', $from_date)
                ->where('leave_application.application_to_date', '<=', $to_date)
                ->where('employee_id', $request->employee_id)
                ->get();
            ...
            // dd($employeeDetails->salary_gross);
            $data = [
                ...
                'employeeGrossSalary'      => $employeeDetails->salary_gross,
            ];
        } else {
            ...
            $employeeHourlySalary = $this->payrollRepository->getEmployeeHourlySalary($request->employee_id, $request->month, 0);
            $data = [
                'employeeList'     => $employeeList,
                // 'hourly_rate'      => $employeeDetails->hourlySalaries->hourly_rate,
                'hourly_rate'      => $employeeDetails->salary / 8,
                'employee_id'      => $request->employee_id,
                'month'            => $request->month,
    ...
    public function calculateEmployeeSalaryDetails($month)
    {
        ...
        foreach ($employeeIds as $key => $id) {
            $employeeDetails = Employee::with('department', 'designation')->where('employee_id', $id)->first(); // 'payGrade', 'hourlySalaries',
        ...
        try {
            ...
            if (isset($request->month) && $current_month != $request->month && $past_month == $request->month) {
                ...
                foreach ($employeeResults as $key => $value) {
                    ...
                    $tax                 = $value['tax'] ?? 0;
                    $basicSalary         = isset($value['employeeDetails']->basic_salary) ? $value['employeeDetails']->basic_salary : 0;
